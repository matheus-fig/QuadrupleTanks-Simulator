...
for k in range(n_steps):
    ...
    # ataque DoS entre 100 < k < 200
    if k > 500: # após k = 500, o simulador ignora as acoes de controle
        pass    # o ultimo valor chegado é mantido ate o fim
    else:
...
for i in range(states.shape[1]):
    plt.step(times, states[:, i], where="post", label=f'h{i+1}')
plt.axvspan(500 * dt, 7000 * dt, color='red', alpha=0.3, label='Ataque DoS')
plt.xlabel('Tempo (s)')
plt.ylabel('Altura (h)')
